/*
 * 该js文件是运行在 Node.js 端的，获取数据的基本的思路就是后端代理，即提供接口路由供前端页面使用。然后在路由内部，我们接收到前端请求后，再发送 HTTP 请求到第三方服务接口，携带相应的请求参数，包括签名的参数字段等等。
...
function get(url, params) {
  return axios.get(url, {
    headers: {
      // Referer 告诉服务器该网页是从哪个页面链接过来的 -- HTTP的header属性
      // 这里在headers中伪造为 https://y.qq.com/ 目的让第三方服务认可这样一个请求
...
// 注册推荐列表接口路由
/*
  拿到express实例（即app）发送get请求到第三方服务，从后者拿到数据后进行处理
  * 其中用到的随机数值，签名值等都是第三方服务所要求的
*/
function registerRecommend(app) {
  app.get('/api/getRecommend', (req, res) => {
    ...
    // 发送 get 请求（这个get函数是对axios的封装，实现代码在40行左右）；现在then这里就可以拿到响应的结果了
    get(url, {
      sign,
      '-': randomVal,
      data
      ...
      if (data.code === ERR_OK) {
        ...
        // 3.（处理完之后）往前端发送一个标准格式的响应数据，包括成功错误码和数据
        //  *这里忽略其它的情况，即只有成功的处理（在实际开发需要处理错误的情况）
        res.json({
          code: ERR_OK,
          result: {
            sliders,
...
function registerSingerList(app) {
  app.get('/api/getSingerList', (req, res) => {
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        ...
        // 定义/构造歌手 Map 数据结构，并保存好
        const singerMap = {
          // 标题 和 singer 的前十条数据作为热门数据的数组
          hot: {
            title: HOT_NAME,
        ...
        // 这里的结果会保存到 singerMap 中（这样它会拥有 hot, A~Z 所有key）
        // * 遍历singerList数组，将歌手按首字母分配各个字母下（单次遍历大约可分3个步骤）
        // *（因为原始数据没有返回字母，它返回的都是歌手名。这里借用第三方库pinyin）
        singerList.forEach((item) => {
          ...
          if (key) {
            ...
            // 3.每个字母下面会有多名歌手（比如给A下的list中添加A姓歌手）
            // 这里会经过map()重新映射新的数据结构后再push进去
            // （备注：因为map()返回的是数组格式，所以这里通过[0]获取item这一条数据）
  ...
  // 【函数封装】做一层数据映射，构造单个 singer 数据结构
  // 逻辑：将歌手列表数组重新映射成新的数组返回
...
function registerSingerDetail(app) {
  app.get('/api/getSingerDetail', (req, res) => {
    ...
    // 构造所需要的请求 data 参数（其中mid是歌手的唯一标识）
    const data = JSON.stringify({
      comm: { ct: 24, cv: 0 },
      singerSongList: {
...
// 注册歌曲 url 获取接口路由
// * 因为歌曲的 url 每天都在变化（就会存在过期情况），所以需要单独的接口根据歌曲的 mid 获取
